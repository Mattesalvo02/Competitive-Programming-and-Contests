...
pub fn next_greater_elements1(nums: Vec<i32>) -> Vec<i32> {
    let n = nums.len();
...
fn main() {
    ...
    let result1 = next_greater_elements1(nums.clone());
    let result2 = next_greater_elements2(nums);
    println!("Brute force: {:?}", result1);
